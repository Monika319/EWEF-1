...
files = ["zad5R30.dat","simzad5R30.dat"]
Dane=[] #empty table
...
Xsim=Dane[1][:, 0]
Ysim=10**(Dane[1][:, 1]/20.)
plt.plot(Xsim, Ysim, "-", label=u"Symulacja Q=16,98")
...
plt.show()
np.savetxt("YsimR30rownolegl",np.transpose([Ysim, Xsim]))
np.savetxt("YexR30rownolegl",np.transpose([Yex, Xex]))
